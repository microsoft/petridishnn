...
from .common import VariableHolder
...
def _compute_pooling_flops(in_shape, pool_size, strides):
    """
    Compute the cost of the pooling measured in multi-add
    """
    try:
        h_pool, w_pool = pool_size
    except:
        h_pool = w_pool = int(pool_size)
    try:
        h_stride, w_stride = strides
    except:
        h_stride = w_stride = int(strides)
    flops = 0.5 # since no multiplication is involved in pooling
    _in_shape = [(x if x is not None else 1) for x in in_shape]
    flops *= float(_in_shape[1] * _in_shape[2] * _in_shape[3])
    flops *= float(h_pool * w_pool)
    flops /= float(h_stride * w_stride)
    return flops
...
def MaxPooling(
    ...
    flops = _compute_pooling_flops(in_shape, pool_size, strides)
...
def GlobalAvgPooling(x, data_format='channels_last'):
    ...
    ret = tf.reduce_mean(x, axis, name='output')
    in_shape = x.get_shape().as_list()
    flops = 0.5 # no multiplication involved
    flops *= float(in_shape[1] * in_shape[2] * in_shape[3])
    ret.info = VariableHolder(flops=flops)
    return ret
