...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help=('Petridish search log paths in a comma separated list.'
             ' The logs are from newest to the oldest of the same run.'
             ' There are multiple logs b/c of interruption/recovery.')
    )
    parser.add_argument('--app_dir',
        default=None, type=str, help='philly_app_dir')
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help='The maximum model iter value allowed'
    )
    ...
    if args.app_dir:
        l_in_fn.extend(app_dir_log_fns(args.app_dir))
    if args.mi:
